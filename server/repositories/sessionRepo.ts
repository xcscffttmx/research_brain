...
const INSERT_SESSION_SQL = `insert into sessions(id, title, created_at, updated_at, summary, summary_upto)
     values (?, ?, ?, ?, '', 0)`;

/** 新建会话 */
export function createSession(title = '新对话'): SessionRow | null {
  const db = getDb();
  const now = Date.now();
  const id = uid('session');
  db.prepare(INSERT_SESSION_SQL).run(id, title, now, now);
 ...
 */
export function ensureSession(id: string, title = '新对话'): SessionRow | null {
  const existing = getSession(id);
  if (existing) return existing;

  const now = Date.now();
  getDb().prepare(INSERT_SESSION_SQL).run(id, title, now, now);
  return getSession(id);
}
...
/** 更新标题并刷新 updated_at */
export function renameSession(id: string, title: string): SessionRow | null {
 ...
 */
export function updateSessionSummary(id: string, summary: string, summaryUpto: number): SessionRow | null {
  getDb()
